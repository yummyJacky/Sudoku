...
class StateNode {
    constructor(grid, parent = null, children = null, now_set_pos = null, pos_valid_branch = new Map(), isInvalid = false, no_solution = false) {
        this.grid = JSON.parse(JSON.stringify(grid));
        this.parent = parent;
        this.children = children;  // 子节点

        this.now_set_pos = now_set_pos;  // 当前设置的位置
        this.pos_valid_branch = pos_valid_branch; // 当前状态在位置(i,j)的合法数  (i,j):[1,2,3,4,5,6,7,8,9]
        this.isInvalid = isInvalid; // 标记状态是否不可行
        this.no_solution = no_solution; // 标记状态是否无解
    }

    clone() {
        // 将对象转换为 JSON 字符串，再解析为新对象
        let serialized = JSON.stringify(this);
        let newObj = JSON.parse(serialized);
        // 恢复 Map 对象
        newObj.pos_valid_branch = new Map(Object.entries(newObj.pos_valid_branch));
        return new StateNode(newObj.grid, newObj.parent, newObj.children, newObj.now_set_pos, this.pos_valid_branch, newObj.isInvalid, newObj.no_solution);
    }
}
...
function createUserGrid(){
    ...
    grid.subscribe($grid => {
        ...
        if (!stateMap.has(initialStateKey)) {
            ...
            initialState.children=null;
            initialState.now_set_pos=null;
        }
        stateMap.set(initialStateKey, initialState.clone());
    ...
    return {
        ...
        set: (pos, value) => {
            userGrid.update($userGrid => {
                ...
                if (!newState) {
                    ...
                    if (newState.now_set_pos&&newState.now_set_pos != pos) {
                    ...
                    newState.now_set_pos = pos;
                    newState.children=null;  //清空弃用的历史操作
                }
                // console.log(pos);
                // console.log(currentState.grid[pos.y][pos.x]);
                // console.log(currentState.grid);
                if (currentState) {
                    // console.log('yes');
                    if (currentState.now_set_pos&&currentState.grid[pos.y][pos.x]!=0) { //改了已确定值的位置，需要继承原节点，而不是作为孩子 否则会出现循环，不是DAG
                        console.log('in');
                        if(currentState.parent) {
                            let currentState_parent = stateMap.get(JSON.stringify(currentState.parent));
                            currentState_parent.children = JSON.parse(JSON.stringify(newState.grid)); //替换路线
                            newState.parent = JSON.parse(JSON.stringify(currentState_parent.grid));
                            stateMap.set(JSON.stringify(currentState_parent.grid), currentState_parent.clone());
                        }else{
                        ...
                        currentState.children = JSON.parse(JSON.stringify(newState.grid)); // 将新状态添加到当前状态的子节点
                        newState.parent = JSON.parse(JSON.stringify(currentState.grid));
                    }
                }
                stateMap.set(JSON.stringify(currentState.grid), currentState.clone());
                stateMap.set(newStateKey, newState.clone());
                currentState = newState.clone(); // 更新当前状态
                return newGrid;
            });
        },
         ...
         */
        undo: () => {
            let prevState = stateMap.get(JSON.stringify(currentState.parent));
            if (prevState) {
                while (prevState.isInvalid || prevState.no_solution) {
                    if (prevState.parent) {
                        prevState = stateMap.get(JSON.stringify(prevState.parent));
                    }else{
                        break;
                    }
                }
                let validBranch;
                if (!prevState.no_solution && !prevState.isInvalid&& prevState.now_set_pos) { //当前有解才进行回溯    不对最初始状态进行回溯，因为最初始状态是有解的
                    ...
                    if (validBranch && Array.isArray(validBranch)) {
                        if (prevState.children && stateMap.get(JSON.stringify(prevState.children)).no_solution) {
                            // 无解状态回传
                        ...
                        if (sum == 0 || prevState.children && stateMap.get(JSON.stringify(prevState.children)).no_solution) {
                            if (sum==0) {
        ...
        ReBarnch: () => {  //回溯到多分枝节点
            let prevState = currentState.clone();
            ...
            if (prevState) {
                ...
                let validBranch;
                if (!(prevState.isInvalid || prevState.no_solution)) { //从合法且有解状态开始
                    while(prevState) {
                        while (prevState.isInvalid || prevState.no_solution) {
                            if (prevState.parent) {
                                prevState = stateMap.get(JSON.stringify(prevState.parent));
                        ...
                        if (!prevState.no_solution && prevState.now_set_pos) { //当前有解才进行回溯    不对最初始状态进行回溯，因为最初始状态是有解的
                            validBranch = prevState.pos_valid_branch.get(`${prevState.now_set_pos.x},${prevState.now_set_pos.y}`);
                            if (validBranch && Array.isArray(validBranch)) {
                                if (prevState.children && stateMap.get(JSON.stringify(prevState.children)).no_solution) {
                                    // 无解状态回传
         ...
         */
        redo: () => {
            let nextState = stateMap.get(JSON.stringify(currentState.children));
            while (nextState && (nextState.isInvalid || nextState.no_solution)) {  //跳过非法状态
                if (nextState.children) {
                    nextState = stateMap.get(JSON.stringify(nextState.children));
                }else{
                    break;
                }
            }
            if (nextState && !nextState.isInvalid && !nextState.no_solution) { //必须合法且有解，否则没有必要redo
        ...
        applyStrategy: (pos, strategy, candidatesStore) => {
            ...
            userGrid.update($userGrid => {
                ...
                if (changes && changes.length > 0) {
                    ...
                    if (currentState) {
                        currentState.no_solution = true;  //无解
                    }
                }
