    ...
    return {
        ...
        applyHint: (pos,strategy) => {
            ...
            userGrid.update($userGrid => {
                ...
                redoHistory = [];
                // 这里根据策略调用不同的solveSudoku，这里默认的是深度优先搜索（DFS）策略，in node_modules\@mattflow\sudoku-solver\index.js
                // 后期在这里调用strategy.effect();实现具体solve
                const solvedSudoku = solveSudoku($userGrid);
                $userGrid[pos.y][pos.x] = solvedSudoku[pos.y][pos.x];
                console.log($userGrid[pos.y][pos.x]);
