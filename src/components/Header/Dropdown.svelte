<script>
    ...
    // 验证输入（支持sencode和URL格式）
    function validateInput(value) {
        return validateSencode(value) || isValidSudokuUrl(value);
    }
	...
	function handleEnterCode() {
		...
		modal.show('prompt', {
			title: 'Enter Code',
			text: 'Please enter a Sudoku code or paste a URL from sudokuwiki.org:',
			fontMono: true,
			button: 'Start',
			onHide: game.resume,
			callback: (value) => {
				if (isValidSudokuUrl(value)) {
					const numbers = extractNumbersFromUrl(value);
					if (numbers) {
						const gridArray = Array.from(numbers).map(Number);
						game.startCustom(gridArray);
					}
				} else if (validateSencode(value)) {
